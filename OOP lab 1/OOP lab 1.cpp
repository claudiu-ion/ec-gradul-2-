...
Complex Complex::comsqrt(const Complex& z) {
    Complex i(0, 1);
    return (Complex)sqrt((modul(z) + z.re) / 2) +  i * (Complex)(z.im / abs(z.im) * sqrt((modul(z) - z.re) / 2));
}
Complex* EcGr2::rez(const EcGr2& p) {
    Complex* x, delta;
    x = new (nothrow) Complex[4];
    Complex i(0, 1);
    delta = (((Complex)p.b * p.b) - ((Complex)"4" * p.a * p.c));
    if (delta == (Complex)"0")
        x[0]= ((Complex)((Complex)"0" -p.b) + delta.Complex::comsqrt(delta)) / ((Complex)"2" * p.a);
    else {
        x[0] = ((Complex)((Complex)"0" -p.b) + delta.Complex::comsqrt(delta)) / ((Complex)"2" * p.a);
        x[1] = ((Complex)((Complex)"0" -p.b) - delta.Complex::comsqrt(delta)) / ((Complex)"2" * p.a);
    }
    return x;
};
...
int main()
{
    ...
    cout << "Solutiile P1:" << endl
        << "x1=" << P1.EcGr2::rez(P1)[0] << endl
        << "x2=" << P1.EcGr2::rez(P1)[1] << endl;
         
    cout << "Solutiile P2:" << endl
        << "x1=" << P2.EcGr2::rez(P2)[0] << endl
        << "x2=" << P2.EcGr2::rez(P2)[1] << endl;
